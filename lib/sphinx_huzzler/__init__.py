...
def create_sitemap(app, exception):
    ...
    print('Generating sitemap.xml in {}'.format(filename))
...
class HTMLTranslator(SphinxHTMLTranslator):
    ...
    def visit_table(self, node, name=''):
        """
        Override docutils default table formatter to not include a border and
    ...
    def visit_field_list(self, node):
        ...
        if 'compact' in node['classes']:
            ...
            self.settings.compact_field_lists and
            'open' not in node['classes']
        ):
            self.compact_field_list = True
